...
- To add a skill mid-engagement, drop it into `.omp/skills/<name>/SKILL.md` and restart
  the session (`kubectl delete session __SESSION_NAME__ -n <session-cr-namespace>` — the
  namespace where the Session CR was created, not necessarily `omp-system` — then re-apply the Session CR).
